/*
 * FOODPANDA WEBSITE - MAIN JAVASCRIPT FUNCTIONALITY
 * This JavaScript file contains all interactive functionality for the foodpanda clone website
 * Includes form handling, navigation, cart functionality, and UI interactions
 */

// Main initialization - Executes when DOM is fully loaded
document.addEventListener('DOMContentLoaded', function() {
    // Initialize all functionality modules
    initializeLocationSearch();           // Handle location search functionality
    initializeCuisineFilters();          // Handle cuisine filter buttons
    initializeMobileMenu();              // Handle mobile menu toggle
    initializeCartFunctionality();       // Handle cart interactions
    initializeSmoothScrolling();         // Handle smooth scrolling for anchor links
    initializeAnimations();              // Handle scroll animations
    initializeTestimonialsCarousel();    // Handle testimonials carousel
    initializeFormValidation();          // Handle form validation
    initializeTabs();                    // Handle tab navigation
    initializeQuantitySelectors();       // Handle quantity selectors
    initializeFavoriteButtons();         // Handle favorite buttons
    initializeFilters();                 // Handle general filters
    initializeOrderTracking();           // Handle order tracking functionality
    initializeProfileFeatures();         // Handle profile page features
    initializeCategoryHoverEffects();    // Handle category card hover effects
    initializeStepHoverEffects();        // Handle how-it-works step hover effects
});

// Location search functionality - Handles location input and restaurant search
...
// Cuisine filter buttons - Handles selection of cuisine filter buttons
...
// Initialize enhanced category card hover effects
    ...
    categoryCards.forEach(card => {
        card.addEventListener('mouseenter', function() {
            ...
            if (icon) {
                ...
                icon.style.background = 'rgba(215, 15, 100, 0.15)';
            }
        });
        ...
        card.addEventListener('mouseleave', function() {
            ...
            if (icon) {
                ...
                icon.style.background = 'rgba(215, 15, 100, 0.08)';
            }
        });
    });
}

// Initialize enhanced step hover effects for How It Works section
function initializeStepHoverEffects() {
